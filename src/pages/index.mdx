...
export default function Index ({ data: { allFilmsJson: { edges: filmEdges }} }) {
	...
	useEffect(() => {
		(async () => {
			...
			stramingProviders.forEach((provdr) => parseStreamingProviders[provdr.id] = { name: provdr.clear_name, key: provdr.technical_name })
			console.log(parseStreamingProviders);
		})();
	}, []);
	return loading ?
	(
